...
int main(int argc, char** argv)
{
    ...
    if (0 == sRc && sPubIn)
    {
        if (sRawIn)
        {
            ...
            if (2336 == sKeyBytes)
            {
                ...
                if (sVerbose) printf("extractdilkey: Found raw public key\n");
                ...
                // Convert public key
                sRc = mlca_key2wire(sKey,    sKeyBytes,
                                    sRawKey, sRawBytes, 0,
                                    NULL, 0,
                                    NULL, 0);
                if (sRc < 0)
        ...
        else
        {
            ...
            sRc = mlca_wire2key(sRawKey, sRawBytes, &sWireType,
                                sKey, sKeyBytes,
                                NULL, ~0);
            if (sVerbose) printf("extractdilkey: Found public key\n");
            // We have a raw Dilithium R2 8x7 public key
            if (2336 != sRc)
            {
    ...
    else if (0 == sRc && !sPubIn)
    {
        if (sRawIn)
        {
            ...
            if (5136 == sKeyBytes)
            {
                if (sVerbose) printf("extractdilkey: Found raw private key\n");
